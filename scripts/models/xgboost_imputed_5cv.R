...
calculate_performance_metrics <- function(true_labels, pred_probs, threshold = 0.5) {
  ...
  return(data.table(
    Accuracy = accuracy, AUC = auc_score, AUPRC = auprc_score, Sensitivity = sensitivity, Specificity = specificity,
...
# ---- Step 4: Loop Through Folds for Imputation, Undersampling, Training, and Evaluation ----
for (i in 1:length(folds)) {
  ...
  for (col in predictor_cols) {
    if (any(is.na(train_imbalanced[[col]]))) {
      ...
      train_imbalanced[is.na(get(col)), (col) := median_val]
      test_df[is.na(get(col)), (col) := median_val]
    }
  }
  
  # ---- Step 4.2: Undersampling ----
  ...
  # ---- Step 4.4: Evaluate on TEST data ----
